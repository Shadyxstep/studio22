...
// Content serving (SPEC §15.1): static rendering + on-demand revalidation.
// DB mode: the current version's content, cached under the "content" tag —
// commits call revalidateContent() and the next request re-reads. File mode
// (no DATABASE_URL): the seed transform of content/*.json, memoized. Either
// way pages render ONE shape, and a DB failure falls back to files so the
// site never goes down with the database.
...
async function readDbContent(): Promise<Content> {
  // Lazy imports keep pg out of the module graph in file mode (and out of
  // client bundles — this module is server-only via next/cache).
...
const readDbContentCached = unstable_cache(readDbContent, ["site-content"], {
  tags: [CONTENT_TAG],
});
...
export async function getContent(): Promise<Content> {
  ...
  try {
    return await readDbContentCached();
  } catch (err) {
...
/** Invalidate the cached content after a commit. Kept here so every write path shares it. */
